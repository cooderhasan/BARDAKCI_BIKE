...
export async function sendOrderInvoice(orderId: string) {
    try {
                ...
                try {
                    if (order.source === "HEPSIBURADA") {
                        ...
                        if (csConfig) {
                            ...
                            if (csResult.success) {
                                return { success: true, message: "Mevcut fatura linki Çiçeksepeti'ye başarıyla yeniden gönderildi ve güncellendi! ✅" };
                            } else {
                ...
                if (order.source === "N11") {
                    ...
                    if (invoiceUrl) {
                        ...
                        if (csConfig) {
                            ...
                            marketplaceMessage = " | Çiçeksepeti entegrasyon ayarı bulunamadı";
                        }
                    } else {
        ...
        if (!invoiceUrl) {
            emailMessage = " | Mail gönderilemedi (PDF Linki Alınamadı)";
        } else if (!customerEmail) {
            emailMessage = " | Mail gönderilemedi (Müşteri E-postası Yok)";
        } else {
            try {
